...
export class OrderService {
  ...
  async create(userId: string, dto: { accountIds: string[]; paymentMethod: 'WALLET'; note?: string }) {
    ...
    if (accounts.length !== dto.accountIds.length) throw new BadRequestException('Some accounts are no longer available');
    ...
    if (balance < total) throw new BadRequestException({ code: 'INSUFFICIENT_BALANCE', required: total, current: balance });
    return this.prisma.$transaction(async (tx) => {
      const order = await tx.order.create({
        data: { userId, totalAmount, paymentMethod: PaymentMethod.WALLET as PrismaPaymentMethod, note: dto.note },
        include: { accounts: true },
      });
      await tx.gameAccount.updateMany({ where: { id: { in: dto.accountIds } }, data: { status: 'RESERVED', orderId: order.id } });
      await tx.user.update({ where: { id: userId }, data: { walletBalance: { decrement: totalAmount } } });
      await tx.walletTransaction.create({ data: { userId, amount: new Prisma.Decimal(-total), type: 'PAYMENT', referenceId: order.id } });
  ...
  async updateStatus(id: string, status: OrderStatus) {
    if (status === 'CANCELLED' || status === 'REFUNDED') {
      await this.prisma.gameAccount.updateMany({ where: { orderId: id }, data: { status: 'AVAILABLE', orderId: null } });
    }
    if (status === 'COMPLETED') {
      await this.prisma.gameAccount.updateMany({ where: { orderId: id }, data: { status: 'SOLD' } });
    }
  ...
  async uploadPaymentProof(id: string, userId: string, paymentProof: string) {
    ...
    if (!order) throw new NotFoundException('Order not found');
    return this.prisma.order.update({ where: { id }, data: { paymentProof } });
  }
  ...
  async getStats() {
    ...
    return { totalAccounts, availableAccounts, totalOrders, ordersToday, revenue: revenue._sum.totalAmount || 0 };
  }
}
