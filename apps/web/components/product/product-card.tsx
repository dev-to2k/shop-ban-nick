  ...
  if (discount && discount > 0) {
    ...
    return (
      <div className="flex items-center gap-2 flex-wrap">
        <span className="font-bold text-primary text-sm tabular-nums">{formatPrice(discounted)}</span>
        <span className="text-xs text-muted-foreground line-through tabular-nums">{formatPrice(price)}</span>
        <Badge variant="destructive" className="text-[0.625rem]">-{discount}%</Badge>
      </div>
    );
  }
  return <span className="font-bold text-primary text-sm tabular-nums">{formatPrice(price)}</span>;
}

export function ProductCard({ item, slug, variant, quantity = 1, onRemove }: ProductCardProps) {
  const cart = useCart();
  ...
  const unitPrice = item.discount && item.discount > 0 ? Math.round(item.price * (1 - item.discount / 100)) : item.price;
        ...
        <CardContent className="p-3 sm:p-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:gap-4">
          <div className="flex items-center gap-3 min-w-0 sm:flex-1">
            <div className="relative aspect-square w-16 sm:w-20 bg-muted rounded-md shrink-0 overflow-hidden flex items-center justify-center">
              ...
              {item.discount && item.discount > 0 && (
                <Badge variant="destructive" className="absolute top-0 right-0 text-[0.625rem] rounded-none rounded-bl">-{item.discount}%</Badge>
              )}
              {item.tag && (
                <Badge className="absolute bottom-0 left-0 text-[0.625rem] rounded-none rounded-tr">{item.tag}</Badge>
              )}
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4 shrink-0">
            <div className="flex items-center gap-1 border rounded-md w-fit">
              <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={() => updateCartQuantity(item.id, Math.max(1, qty - 1))} aria-label="Giảm số lượng">
                <Minus className="h-4 w-4" />
              </Button>
              <span className="min-w-[2ch] text-center text-sm font-medium tabular-nums" aria-live="polite">{qty}</span>
              <Button variant="ghost" size="icon" className="h-8 w-8 shrink-0" onClick={() => updateCartQuantity(item.id, qty + 1)} aria-label="Tăng số lượng">
            ...
            <div className="flex items-center justify-between gap-2 sm:justify-end">
              <div className="min-w-0">
                {qty > 1 ? (
                  <div className="flex flex-col">
                    <span className="font-bold text-primary text-sm tabular-nums">{formatPrice(lineTotal)}</span>
                    <span className="text-xs text-muted-foreground tabular-nums">{formatPrice(unitPrice)} × {qty}</span>
                  </div>
                ) : (
                  priceEl
                )}
              ...
              <Button
                ...
                onClick={() => (onRemove ? onRemove(item.id) : removeFromCart(item.id))}
  ...
  return (
    <Card className="group hover:shadow-lg transition-shadow flex flex-col h-full min-w-0">
      <div className="relative w-full shrink-0 overflow-hidden rounded-t-xl">
        {imgSrc ? (
          <CardImage
            ...
            fallback={<span className="text-2xl font-bold text-muted-foreground">{item.code}</span>}
          />
        ) : (
        ...
        {item.tag && (
          <Badge className="absolute top-2 left-2 z-10 text-[0.625rem]">{item.tag}</Badge>
        )}
        {item.discount && item.discount > 0 && (
          <Badge variant="destructive" className="absolute top-2 right-2 z-10 text-[0.625rem]">-{item.discount}%</Badge>
        )}
      </div>

      <CardContent className="p-3 flex flex-col flex-1">
        <Link href={`/games/${hrefSlug}/${item.id}`} className="hover:underline">
          <h3 className="font-semibold text-sm line-clamp-2 min-h-10">{item.title}</h3>
        </Link>
        ...
        <div className="flex flex-wrap gap-1 mt-2 min-h-12 overflow-hidden content-start">
          {item.attributes &&
            Object.entries(item.attributes).slice(0, 3).map(([key, val]) => (
              <Badge key={key} variant="secondary" className="text-[0.625rem] h-5">{key}: {String(val)}</Badge>
            ))}
        </div>
      </CardContent>
