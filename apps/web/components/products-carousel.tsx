...
import { CarouselNavButton, carouselNavButtonPositions } from '@/components/carousel-nav-button';
import { Button } from '@/components/ui/button';
import { api } from '@/lib/api';
import { queryKeys } from '@/lib/query-keys';
import type { ProductItemDisplay } from '@/components/product/product-card';
...
const SLIDE_CLASS = 'flex-[0_0_min(280px,85vw)] sm:flex-[0_0_min(300px,45vw)] md:flex-[0_0_320px] shrink-0';
...
function ProductsCarouselMulti(props: ProductsCarouselMulti) {
  ...
  const items = useMemo(() => {
    ...
    accountQueries.forEach((q, i) => {
      ...
      if (!game) return;
      const accounts = (q.data as { data?: unknown[] } | undefined)?.data ?? [];
      accounts.forEach((acc: Parameters<typeof mapAccountToItem>[0]) => {
...
function ProductsCarouselSingle(props: ProductsCarouselSingle) {
  ...
  const items = useMemo(() => {
    const accounts = (data as { data?: unknown[] } | undefined)?.data ?? [];
    return accounts.map((acc: Parameters<typeof mapAccountToItem>[0]) => ({
