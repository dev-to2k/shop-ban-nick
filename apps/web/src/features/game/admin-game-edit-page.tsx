...
import { z } from 'zod';
import { type UpdateGameInput } from '@shop-ban-nick/shared-schemas';
import { ArrowLeft, Plus, X, Save } from 'lucide-react';
import { Button, Card, CardContent, CardHeader, CardTitle, Form, FormInput, FormTextarea, FormSelect, FormCheckbox, FormSubmitError, Skeleton } from '@shared/components';
import { api, queryKeys } from '@shared/api';
import { useAdminGame } from '@shared/hooks';
...
type GameEditFormValues = z.infer<typeof gameEditFormSchema>;

function toUpdateGamePayload(data: GameEditFormValues): Parameters<typeof api.admin.updateGame>[1] {
  return {
    ...data,
...
export function AdminGameEditPage({ gameId }: { gameId: string }) {
  ...
  const updateMutation = useMutation({
    mutationFn: (data: GameEditFormValues) => api.admin.updateGame(gameId, toUpdateGamePayload(data)),
    onSuccess: () => {
  ...
  const onSubmit = (data: GameEditFormValues) => {
  ...
  if (isLoading) {
    return (
      <div className="max-w-2xl space-y-4">
        ...
        <Card>
          <CardHeader><CardTitle className="text-base">Thuộc tính acc</CardTitle></CardHeader>
