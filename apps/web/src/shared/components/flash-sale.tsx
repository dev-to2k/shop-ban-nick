...
import { CountdownBlock } from './countdown-slot';
  ...
  return (
    <div className="mt-2">
      ...
      <div className="h-2 bg-muted rounded-full overflow-hidden">
        <div
          className={`h-full rounded-full transition-[width] duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] ${isHot ? 'bg-destructive' : 'bg-primary'}`}
          style={{ width: `${percent}%` }}
        />
      </div>
    </div>
    ...
    <section className={sectionCls} aria-labelledby="flash-sale-heading">
      <div className="container-narrow">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
          ...
          {/* Countdown — flip animation per digit */}
          <div className="flex items-center gap-2 text-sm">
            <Clock className="h-4 w-4 text-muted-foreground shrink-0" />
            <span className="text-muted-foreground shrink-0">Kết thúc sau:</span>
            <div className="flex gap-1 font-mono font-bold text-background tabular-nums">
              <div className="inline-flex items-center justify-center bg-foreground rounded min-w-9 h-7 px-2 overflow-hidden">
                <CountdownBlock value={hours} />
              </div>
              <span>:</span>
              <div className="inline-flex items-center justify-center bg-foreground rounded min-w-9 h-7 px-2 overflow-hidden">
                <CountdownBlock value={minutes} />
              </div>
              <span>:</span>
              <div className="inline-flex items-center justify-center bg-foreground rounded min-w-9 h-7 px-2 overflow-hidden">
                <CountdownBlock value={seconds} />
              </div>
            </div>
          </div>
        </div>
            ...
            <div className="flex gap-4">
              {accounts.map((acc) => {
                ...
                return (
                  ...
                  >
                    <Link
                      href={`/games/${acc.game?.slug}/${acc.id}`}
                      className="block focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 rounded-xl"
                    >
                      <Card className="group hover:shadow-lg transition-shadow h-full min-h-[320px] flex flex-col active:scale-[0.99] transition-transform">
                        <div className="relative">
                          <CardImage
                        ...
                        <CardContent className="p-5 flex flex-col gap-4">
                          <div className="flex flex-col gap-2 h-12 shrink-0">
