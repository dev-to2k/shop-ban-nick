...
import { padZero } from '@shared/hooks/use-countdown-slot';
import { Badge } from './badge';
import { SlotDigit } from './countdown-slot';
  ...
  return (
    <Card className="flex flex-col h-full overflow-hidden">
      ...
      <CardContent className="p-5 flex flex-col flex-1 gap-4">
        <div className="flex flex-col gap-2 h-13 shrink-0">
      ...
      <CardFooter className="px-5 py-4 flex items-center justify-between gap-4 border-t shrink-0">
...
export function ProductCard({
  ...
  if (variant === 'cart') {
    ...
    return (
      <Card>
        <CardContent className="p-5 flex flex-col gap-4 sm:flex-row sm:items-center sm:gap-4">
          <div className="flex items-center gap-3 min-w-0 sm:flex-1">
            ...
            <div className="flex-1 min-w-0 flex flex-col gap-2">
              ...
              <h3 className="text-base font-semibold leading-tight line-clamp-2 sm:truncate">
                {item.title}
              </h3>
              <p className="text-sm text-muted-foreground">Mã: {item.code}</p>
            </div>
          </div>
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 sm:gap-4 shrink-0">
            <div className="flex items-center gap-1 border rounded-md w-fit">
              ...
              <span
                className="flex items-center justify-center min-w-[2ch] text-sm font-medium tabular-nums"
                aria-live="polite"
              >
                <SlotDigit
                  value={parseInt(padZero(Math.min(99, qty))[0], 10)}
                  className="relative inline-block h-5 min-w-[1ch] overflow-hidden text-center leading-5 text-sm"
                  rowClassName="h-5 shrink-0 flex items-center justify-center text-sm"
                  rowHeightRem={1.25}
                />
                <SlotDigit
                  value={parseInt(padZero(Math.min(99, qty))[1], 10)}
                  className="relative inline-block h-5 min-w-[1ch] overflow-hidden text-center leading-5 text-sm"
                  rowClassName="h-5 shrink-0 flex items-center justify-center text-sm"
                  rowHeightRem={1.25}
                />
  ...
  return (
    <Card className="group hover:shadow-lg hover:-translate-y-0.5 active:scale-[0.99] transition-all duration-300 flex flex-col h-full min-w-0 focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2 focus-within:outline-none">
      <div className="relative w-full shrink-0 overflow-hidden rounded-t-xl">
        ...
        {item.discount && item.discount > 0 && (
          <Badge variant="discount" className="absolute top-2 right-2 z-10">
            -{item.discount}%
          </Badge>
        )}
      </div>

      <CardContent className="p-5 flex flex-col flex-1 gap-4 min-h-[10rem]">
        <div className="flex flex-col gap-2 h-13 shrink-0">
        ...
        <div className="flex flex-wrap gap-2 min-h-12 overflow-hidden content-start">
          {item.attributes &&
      ...
      <CardFooter className="px-5 py-4 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between mt-auto shrink-0 border-t">
        <div className="min-w-0 text-sm">{priceEl}</div>
        <Link
