...
import { User, Mail, Phone, Shield, Package, Wallet, History, Camera } from 'lucide-react';
      ...
      <Card className="max-w-md mx-auto">
        <CardHeader className="text-center">
          ...
          <div className="mx-auto relative inline-block mb-2">
            ...
            >
              {avatarUrl ? (
                <Image src={avatarUrl} alt="" width={64} height={64} className="w-full h-full object-cover" unoptimized sizes="64px" />
              ) : (
          ...
          <Link href="/profile/wallet" className="block">
            <Button variant="outline" className="w-full">
              ...
              {typeof u.walletBalance === 'number' && (
                <span className="ml-2 text-primary font-semibold tabular-nums">({formatPrice(u.walletBalance)})</span>
              )}
            </Button>
          </Link>
          <Link href="/profile/transactions" className="block">
            <Button variant="outline" className="w-full">
              <History className="h-4 w-4 mr-2" />Lịch sử giao dịch
            </Button>
          </Link>
          <Link href="/orders" className="block">
            <Button variant="outline" className="w-full">
              <Package className="h-4 w-4 mr-2" />Đơn hàng của tôi
            </Button>
          </Link>
        </CardContent>
      </Card>
