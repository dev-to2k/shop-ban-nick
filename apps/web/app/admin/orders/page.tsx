                    ...
                    <div className="mt-4 pt-4 border-t space-y-3">
                      ...
                      <div className="flex gap-2">
                        {order.status === 'PENDING' && (
                          <>
                            <Button size="sm" onClick={() => statusMutation.mutate({ id: order.id, status: 'CONFIRMED' })} disabled={statusMutation.isPending}>
                              <CheckCircle className="h-4 w-4 mr-1" /> Xác nhận
                            </Button>
                            <Button size="sm" variant="destructive" onClick={() => statusMutation.mutate({ id: order.id, status: 'CANCELLED' })} disabled={statusMutation.isPending}>
                              <XCircle className="h-4 w-4 mr-1" /> Hủy
                            </Button>
                          </>
                        )}
                        {order.status === 'CONFIRMED' && (
                          <Button size="sm" onClick={() => statusMutation.mutate({ id: order.id, status: 'COMPLETED' })} disabled={statusMutation.isPending}>
